...
export type StagehandInitScriptSource<Arg = unknown> =
  | string
  | { path?: string; content?: string }
  | ((arg: Arg) => unknown);

export type StagehandInitScript<Arg = unknown> =
  | StagehandInitScriptSource<Arg>
  | {
      script: StagehandInitScriptSource<Arg>;
      arg?: Arg;
    };
...
export interface V3Options {
  ...
  /**
   * JavaScript to inject into every page before page scripts run.
   *
   * These scripts are registered with the browser context during init, before
   * control is returned to user code. This is useful for agent instrumentation,
   * environment shims, script blocking, or page markers that must exist before
   * the target document evaluates its own JavaScript.
   */
  initScripts?: StagehandInitScript[];
  disableAPI?: boolean;
  /**
