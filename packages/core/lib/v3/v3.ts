...
export class V3 {
      ...
      return await withInstanceLogContext(this.instanceId, async () => {
        ...
        if (this.opts.env === "LOCAL") {
          ...
          if (lbo.cdpUrl) {
            ...
            await this._installConfiguredInitScripts();
  ...
  private async _installConfiguredInitScripts(): Promise<void> {
    if (!this.ctx || !this.opts.initScripts?.length) return;

    for (const initScript of this.opts.initScripts) {
      if (this._isInitScriptWithArg(initScript)) {
        await this.ctx.addInitScript(initScript.script, initScript.arg);
      } else {
        await this.ctx.addInitScript(initScript);
      }
    }
  }

  private _isInitScriptWithArg(
    initScript: StagehandInitScript,
  ): initScript is {
    script: Exclude<StagehandInitScript, { script: unknown }>;
    arg?: unknown;
  } {
    return (
      typeof initScript === "object" &&
      initScript !== null &&
      "script" in initScript
    );
  }
